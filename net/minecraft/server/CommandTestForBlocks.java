                    ...
                    for (int j = structureboundingbox.c; j <= structureboundingbox.f; ++j) {
                        for (int k = structureboundingbox.b; k <= structureboundingbox.e; ++k) {
                            for (int l = structureboundingbox.a; l <= structureboundingbox.d; ++l) {
                                blockposition_a.c(l, k, j);
                                blockposition_a1.c(l + blockposition3.getX(), k + blockposition3.getY(), j + blockposition3.getZ());
                                boolean flag1 = false;
                                IBlockData iblockdata = world.getType(blockposition_a);

                                if (!flag || iblockdata.getBlock() != Blocks.AIR) {
                                    if (iblockdata == world.getType(blockposition_a1)) {
                                        TileEntity tileentity = world.getTileEntity(blockposition_a);
                                        TileEntity tileentity1 = world.getTileEntity(blockposition_a1);
